...
console.log('userData----------', userData);
  ...
  const [visibleServicesIds, setVisibleServicesIds] = useState<boolean>(false);
  ...
  const onAdd = () => {
    ...
    if (name == '') {
      ...
      for (let i = 0; i < servicesIds.length; i++) {
        services.push(servicesIds[i].id);
      };
  ...
  const addNewEmployeeSection = () => {
    var array_service: string = '';
      var array_features: string = '';
      var array_facilities: string = '';

      for (let m = 0; m < servicesIds.length; m++) {
        if (m == 0) {
          array_service = `${servicesIds[m].name}`;
        } else {
          array_service = `${array_service} - ${servicesIds[m].name}`;
        }
      };
    return (
      <Modal
        ...
        <View style={styles.modalAddContainer}>
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <AppPickerSelect
              ...
              touchContainerStyle={{width: calcWidth(343)}}
              styleTitle={{}}
              onPress={() => setVisibleServicesIds(true)}
              title={Trans('service')}
              placeholder={array_service != '' ? array_service : Trans('service')}
              icon={IMAGES.dropDown}
            />
  ...
  const modalSelectService = () => {
    return (
      <AppPickerMultiSelect
        visible={visibleServicesIds}
        onClose={() => {setVisibleServicesIds(false)}}
        onSelectItem={(array) => {setServicesIds(array)}}
        title={Trans('chooseServices')}
        data={serviceData}
        itemSelected={servicesIds}
      />
    )
  }
